...
use Linfo\OS\Unixcommon;
use Exception;

class LinuxNoCpuPercentage extends Unixcommon
{
    ...
    public function getTemps()
    {
        ...
        if (array_key_exists('hddtemp', (array) $this->settings['temps']) && !empty($this->settings['temps']['hddtemp']) && isset($this->settings['hddtemp'])) {
            try {
                ...
                if ($this->settings['hddtemp']['mode'] == 'daemon') {
                    $hddtemp->setAddress(
                            $this->settings['hddtemp']['address']['host'], $this->settings['hddtemp']['address']['port']);
                }
        ...
        if (array_key_exists('mbmon', (array) $this->settings['temps']) && !empty($this->settings['temps']['mbmon']) && isset($this->settings['mbmon'])) {
            try {
                ...
                // Set host and port
                $mbmon->setAddress(
                        $this->settings['mbmon']['address']['host'], $this->settings['mbmon']['address']['port']);
